...
func TestBasic4A(t *testing.T) {
	...
	tester.AnnotateTest("TestBasic4A", N_SERVERS)
...
func TestConcurrent4A(t *testing.T) {
	...
	tester.AnnotateTest("TestConcurrent4A", N_SERVERS)
	...
	wg.Wait()
	ts.checkCounter(N_INC, N_SERVERS)
}
...
func TestLeaderFailure4A(t *testing.T) {
	...
	tester.AnnotateTest("TestLeaderFailure4A", N_SERVERS)
	...
	r = ts.oneInc()
	ts.checkCounter(r.N, N_SERVERS-1)
...
func TestLeaderPartition4A(t *testing.T) {
	...
	tester.AnnotateTest("TestLeaderPartition4A", N_SERVERS)
	...
	go func() {
		...
		for i := 0; i < NSUBMIT; i++ {
			...
			go func(i int) {
				defer wg.Done()
				if err, rep := ts.servers[l].rsm.Submit(Dec{}); err == rpc.OK {
	...
	ts.checkCounter(rep.(*IncRep).N, N_SERVERS)
}
...
func TestRestartReplay4A(t *testing.T) {
	...
	tester.AnnotateTest("TestRestartReplay4A", N_SERVERS)
	...
	if r.N != N_INC+1 {
		err := fmt.Sprintf("expected %d got %d", N_INC+1, r.N)
		tester.AnnotateCheckerFailure(err, err)
		t.Fatalf(err)
	}
...
func TestShutdown4A(t *testing.T) {
	...
	tester.AnnotateTest("TestShutdown4A", N_SERVERS)
	...
	select {
	case <-done:
	case <-time.After((N_SECONDS + 1) * time.Second):
...
func TestRestartSubmit4A(t *testing.T) {
	...
	tester.AnnotateTest("TestRestartSubmit4A", N_SERVERS)
...
func TestSnapshot4C(t *testing.T) {
	...
	tester.AnnotateTest("TestSnapshot4C", N_SERVERS)
	ts.Begin("Test creating and restoring snapshot")

	for i := 0; i < N; i++ {
		ts.oneInc()
	}
	ts.checkCounter(N, N_SERVERS)
